-- 礼品系统第一版业务菜单：商品管理、卡密管理、礼品卡管理。
-- 可重复执行。执行后请退出并重新登录，以重新拉取动态路由。
USE gift_admin;
SET NAMES utf8mb4;

-- 第一版尚未开发权益方案和发放管理，清理其菜单及角色关联。
DELETE FROM `sys_role_menu` WHERE `menu_id` IN (103, 104, 105);
DELETE FROM `sys_menu` WHERE `id` IN (103, 104, 105);
...
INSERT INTO `sys_menu` (`id`, `parent_id`, `tree_path`, `name`, `type`, `route_name`, `route_path`, `component`, `perm`, `always_show`, `keep_alive`, `visible`, `sort`, `icon`, `redirect`, `create_time`, `update_time`, `params`) VALUES
  ...
  (102, 0, '0', '礼品卡管理', 'M', 'GiftCard', '/gift-cards', 'gift/card/index', NULL, 0, 1, 1, 3, 'tickets', NULL, NOW(), NOW(), NULL)
...
-- 第一版不使用按钮权限，只关联三个业务菜单。
INSERT IGNORE INTO `sys_role_menu` (`role_id`, `menu_id`) VALUES (1, 100), (1, 101), (1, 102);
